...
def generate_image_julia( parameter, center=-0.7435 + 0.1314j, width=0.002, max_iterations=256, escape_count=1000, dimension=256, colormap=False, map='twilight'):
    julia_set = JuliaSet(max_iterations=10, escape_radius=1000, parameter=parameter)
    if not colormap:
        image = Image.new(mode="L", size=(dimension, dimension))
    else:
    ...
    image.save(os.path.join("assets", "julia") + ".jpg")
    return viewport
